...
import ModalCreateFreelancer from 'components/Modals/modalcreatefreelancers';
...
import ModalEditFreelancer from 'components/Modals/modaleditfreelancers';
...
function deleteelement(e,id2) {
    e.preventDefault();
    axios.delete(`https://criptoadviser.com/api/freelancers/${id2}`,{ headers: {'Accept': 'application/json','Content-Type': 'application/json' }})
    .then(() => {
      asd()
      })
...
const {globalFilter} = state
  ...
  return (
    ...
    >
      <ModalCreateFreelancer asd={asd} setOpen={setOpen} ></ModalCreateFreelancer>
    </form>
          </div>
        </Fade>
      </Modal>
    ...
    >
      <ModalEditFreelancer asd={asd} setOpen={setOpen2} object={info}></ModalEditFreelancer>
    </form>
          </div>
        </Fade>
      </Modal>
            ...
            <Grid container alignItems="center" style={{padding:5}}>
              ...
              <h3 style={{margin:0}}>Freelancers</h3>
              <IconButton  
              color="primary" 
              variant="contained" 
                  ...
                  {row.cells.map((cell,index) => {
                    return (
                      <TableCell key={index} {...cell.getCellProps()}>
                        {index === 0 ? <img alt="img" src={cell.value} style={{width:200}}></img> : index === 3 ? 
                        <Grid container>
                             <Grid md={12} sm={12}>
