...
function deleteelement(e,id2) {
    e.preventDefault();
    axios.delete(`https://criptoadviser.com/api/products/${id2}`,{ headers: {'Accept': 'application/json','Content-Type': 'application/json' }})
    .then(() => {
      asd()
      })
...
const {globalFilter} = state
  ...
  return (
    ...
    >
      <ModalCreateProduct asd={asd} setOpen={setOpen} ></ModalCreateProduct>
    </form>
          </div>
        </Fade>
      </Modal>
    ...
    >
      <ModalEditProduct asd={asd} setOpen={setOpen2} info={info}></ModalEditProduct>
    </form>
          </div>
        </Fade>
      </Modal>
