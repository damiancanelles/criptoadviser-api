...
export default function ModalEditClaseI({setOpen, asd, info, id}) {
    const [title,settitle] = React.useState(info.titulo);
    const [descripcion,setdescripcion] = React.useState(info.descripcion);
    const [link,setlink] = React.useState(info.link);
    const [datestart,setdatestart] = React.useState(info.datestart.split("T")[0]);
    const [inicio,setinicio] = React.useState(info.duracion.split(" - ")[0]);
    const [final,setfinal] = React.useState(info.duracion.split(" - ")[1]);
    const [cosa,setcosa] = React.useState(info.file);
    ...
    async function register(e) {
        ...
        if (cosa === info.file) {
            const qwe = {
              ...
              axios.put(`https://criptoadviser.com/api/clasesI/${info._id}`,qwe,{ headers: {'Accept': 'application/json','Content-Type': 'application/json' }})
              .then((res) => {
